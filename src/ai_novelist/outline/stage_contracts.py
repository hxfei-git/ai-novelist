...
STAGE_CONTRACTS = {
    ...
    "chapter_outline": StageContract(
        key="chapter_outline",
        label="章节大纲",
        purpose="按卷渐进生成章节大纲：每次拆解并确认一整卷，全部卷完成后才进入审稿锁定；每章先判定章级功能 profile，再按稳定结构灵活填充。",
        canon_policy="locked_only",
        allowed_intents=(
            "按卷生成",
            "卷内章节总体规划",
        ...
        slots=(
            StageSlot("volume_plan", "卷内章节总体规划", "本卷章节总数、区间任务、高潮/反转/爽点/伏笔/信息揭示/情绪曲线和钩子分布", True, 8, 220),
            StageSlot("chapter_table", "章节列表总表", "每章编号、标题、profile、PacingTarget、一句话概括、核心事件、人物、功能、情绪、结尾状态", True, 40, 180),
            StageSlot("chapter_profile", "章级功能 profile", "开篇、铺垫、过渡、冲突、爽点、反转、揭秘、感情、世界观、战斗、谋略、日常、高潮、收束、钩子", True, 40, 80),
            StageSlot("stable_modules", "单章稳定结构", "基础定位、剧情执行、人物关系读者认知、伏笔爽点情绪开头结尾、世界观资源代价阵营连续性、执行审稿", True, 40, 240),
    ...
    "review_lock": StageContract(
        key="review_lock",
        label="审稿锁定",
        purpose="审计前序阶段的继承链、锁定来源与未决风险，区分阻塞型结构问题和非阻塞细节问题，给出最终锁定判定、回改阶段与章节卡准备度。",
        ...
        forbidden_intents=("新设定", "新人物", "新剧情重写", "候选 canon", "无来源稳定项", "章节卡正文"),
        slots=(
            StageSlot("consistency", "阶段承接检查", "前序阶段是否继承一致，哪里需要回改", True, 8, 100),
            StageSlot("locked_canon", "已锁定 canon 清单", "可进入写作的稳定项，每条必须给出来源阶段或锁定产物", True, 12, 120),
            StageSlot("risk_grading", "风险分级", "阻塞型结构问题与非阻塞细节问题分开列出", True, 8, 110),
            StageSlot("fix_stages", "需要回改的阶段", "按优先级列出需要回改的阶段", False, 4, 110),
            StageSlot("ready", "是否可进入章节卡", "pass/revise/stop", True, 1, 40),
            StageSlot("open_questions", "仍需确认的问题", "只保留不能自动闭环的待确认点", False, 6, 110),
        ),
        confirmation_policy="只问能否锁定、哪些阶段需回改、哪些风险必须保留到章节卡前处理；不把候选内容写成已锁定设定。",
        max_questions=MAX_STAGE_CONFIRMATION_QUESTIONS,
