...
- Files changed so far: `tests/test_web_service.py`, `src/ai_novelist/web/outline_service.py`, `src/ai_novelist/web/outline_actions.py`, docs, and `docs/superpowers/plans/2026-05-29-outline-review-apply-completion.md`.
- Behavior changed: outline overall-review apply now persists terminal applied metadata on the report JSON/Markdown pair, direct apply responses return the same applied metadata, latest-review returns that applied state after refresh, and duplicate apply for the same `run_id` returns `already_applied: true` without rerunning the revision flow.
...
- Verification: RED first failed with `latest["status"]` still `revision_requested` and no `already_applied` key on duplicate apply. Targeted backend/API verification passed with `.venv/bin/python -m pytest tests/test_web_service.py::test_outline_review_apply_marks_latest_report_applied tests/test_web_service.py::test_apply_outline_review_is_idempotent_after_report_applied tests/test_web_service.py::test_apply_outline_review_uses_recommended_custom_and_skip_decisions tests/test_web_service.py::test_apply_outline_review_rejects_empty_custom_decision tests/test_web_app.py::test_outline_review_apply_route_accepts_decisions -q` -> 5 passed in 0.43s after the follow-up metadata/comparator correction.
- Remaining risk: frontend work is still required to stay on 总体审查, hide stale controls, and show completed/applying states; no frontend build or browser-level verification was run in this backend task.
