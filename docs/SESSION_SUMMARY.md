...
- Director 交互转译增强：所有用户输入仍先进入 Director；Director prompt 现在包含最近编辑意见和待确认项，并能把“答案 + 接收/接受/同意”的多项确认合并成下游可执行约束。新一轮 turn 会清掉上一次残留的 `error`，避免历史超时在后续成功回复里反复出现；`quit/exit/退出/stop` 现在会在进入决策前直接结束当前 turn，避免被 outline 阶段继续消费。
