...
- Files changed so far: `tests/test_web_service.py`, `src/ai_novelist/web/outline_service.py`, `src/ai_novelist/web/outline_actions.py`, `tests/test_frontend_review_tabs_structure.py`, `web/frontend/src/types.ts`, `web/frontend/src/main.tsx`, `web/frontend/src/workspaces/review.tsx`, `web/frontend/src/styles.css`, docs, and `docs/superpowers/plans/2026-05-29-outline-review-apply-completion.md`.
- Behavior changed: outline overall-review apply now persists terminal applied metadata on the report JSON/Markdown pair, direct apply responses return the same applied metadata, latest-review returns that applied state after refresh, and duplicate apply for the same `run_id` returns `already_applied: true` without rerunning the revision flow. The frontend now remains on `大纲 > 总体审查`, shows `正在采纳` with a spinner while apply is running, disables duplicate apply/dismiss after completion, and hides stale repair choices behind a `采纳完成` state.
- Backend implementation note: follow-up review removed the unrequested scoped `version_comparator` call; idempotency coverage now asserts duplicate apply avoids a second reviser run and returns persisted applied metadata.
- Verification: backend RED first failed with `latest["status"]` still `revision_requested` and no `already_applied` key on duplicate apply; targeted backend/API verification passed with `.venv/bin/python -m pytest tests/test_web_service.py::test_outline_review_apply_marks_latest_report_applied tests/test_web_service.py::test_apply_outline_review_is_idempotent_after_report_applied tests/test_web_service.py::test_apply_outline_review_uses_recommended_custom_and_skip_decisions tests/test_web_service.py::test_apply_outline_review_rejects_empty_custom_decision tests/test_web_app.py::test_outline_review_apply_route_accepts_decisions -q` -> 5 passed in 0.43s after the follow-up metadata/comparator correction. Frontend RED first failed on the four expected missing applied-state/UI/style assertions; after implementation, `.venv/bin/python -m pytest tests/test_frontend_review_tabs_structure.py::test_outline_review_apply_stays_on_review_after_success tests/test_frontend_review_tabs_structure.py::test_outline_review_type_exposes_applied_metadata tests/test_frontend_review_tabs_structure.py::test_outline_review_workspace_renders_apply_completion_state tests/test_frontend_review_tabs_structure.py::test_outline_review_apply_spinner_styles_exist -q` passed with 4 passed in 0.03s, and `npm --prefix web/frontend run build` passed with the known Vite CJS Node API deprecation warning.
- Remaining risk: affected-suite/full pytest verification remains pending for Task 3; no live browser/SSE click test was added, so the spinner/completion flow is covered by source-structure tests plus TypeScript build only.
