...
### Follow-up: Outline Review Priority Audit Progress

- Files changed so far: `src/ai_novelist/web/outline_service.py`, `src/ai_novelist/prompts/outline_editor.md`, `tests/test_web_service.py`, `tests/test_prompt_loader.py`, plus docs.
- Behavior changed: overall outline review parses high/low/suggestion priority sections with caps of 50, 20, and 10 respectively; legacy flat or paired reports default to low priority; the outline editor prompt requests priority sections; selected apply decisions are rendered in priority order, including custom decisions, with stable submitted ordering inside the same priority.
- Verification so far: Task 1 parser focused tests passed; full `tests/test_web_service.py` passed with 88 passed; prompt loader verification passed with 12 passed; Task 3 focused apply verification passed with 5 passed.
- Remaining risk: frontend grouping/defaults and final verification are still pending.
