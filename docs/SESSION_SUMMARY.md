...
- Behavior changed: outline overall-review apply now persists terminal applied metadata on the report JSON/Markdown pair, latest-review returns that applied state after refresh, and duplicate apply for the same `run_id` returns `already_applied: true` without rerunning the revision/comparison flow.
- Backend implementation note: outline-review apply performs the scoped `version_comparator` prompt call before marking the report applied because the shared graph comparison node currently uses a local fallback and does not invoke the adapter.
